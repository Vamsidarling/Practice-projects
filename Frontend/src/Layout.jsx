...
const handleDisconnect = async () => {
    try {
        const response = await axios.get('http://localhost:3000/user/auth/twitter/disconnetct',{
    ...
    const checkTwitterStatus = async () => {
      try {
        const response = await fetch(
          "http://localhost:3000/user/auth/twitter/status",
          {
            credentials: "include",
          }
        );
  ...
  const handleTwitterAuth = async () => {
    if (isTwitterConnected) {
      ...
      try {
        const response = await fetch(
          "http://localhost:3000/user/auth/twitter/oauth1/request-token"
        );
        const data = await response.json();
        if (data.url) {
